...
function parseCsvSurvey(text) {
  ...
  const latIndex = firstMatchingIndex(headers, ["lat", "latitude", "gpslatitude", "latitudedeg", "latitudedegrees"]);
  const lngIndex = firstMatchingIndex(headers, ["lon", "lng", "long", "longitude", "gpslongitude", "longitudedeg", "longitudedegrees"]);
...
function collectGeoJsonPoints(input) {
  ...
  function addPoint(coords, props = {}) {
    ...
    points.push({
      lat, lng,
      cpm: numeric(props.cpm ?? props.CPM ?? props.counts_per_minute),
      cps: numeric(props.cps ?? props.CPS ?? props.counts_per_second),
      doseRate: numeric(props.dose_rate ?? props.doseRate ?? props.usvh ?? props.uSv_h ?? props["µSv/h"]),
      timestamp: props.timestamp ?? props.datetime ?? props.time ?? null
    });
  }
  function walk(feature) {
    if (!feature) return;
    if (Array.isArray(feature)) {
      feature.forEach(walk);
      return;
    }
    if (feature.type === "FeatureCollection") {
      (feature.features || []).forEach(walk);
      return;
    }
    if (feature.type === "Feature") {
      const geometry = feature.geometry || {};
      const properties = feature.properties || {};
      if (geometry.type === "Point") addPoint(geometry.coordinates, properties);
      else if (geometry.type === "LineString" || geometry.type === "MultiPoint") geometry.coordinates.forEach(coords => addPoint(coords, properties));
      else if (geometry.type === "MultiLineString") geometry.coordinates.flat().forEach(coords => addPoint(coords, properties));
      return;
    }
    if (feature.type && feature.coordinates) {
      walk({ type: "Feature", geometry: feature, properties: {} });
      return;
    }
    const lat = Number(feature.lat ?? feature.latitude ?? feature.gpsLatitude);
    const lng = Number(feature.lng ?? feature.lon ?? feature.long ?? feature.longitude ?? feature.gpsLongitude);
    if (Number.isFinite(lat) && Number.isFinite(lng)) {
      addPoint([lng, lat], feature);
    }
  }
  walk(input);
  if (!points.length) throw new Error("No point or line coordinates found in JSON or GeoJSON.");
  return points;
}

function findXmlValue(node, names) {
  const wanted = names.map(normalizeHeader);
  for (const element of node.querySelectorAll("*")) {
    if (wanted.includes(normalizeHeader(element.localName))) return element.textContent;
  }
  return null;
}
...
function parseGpxSurvey(text) {
  ...
  const points = nodes.map(node => {
    ...
    return {
      ...
      timestamp: findXmlValue(node, ["time", "timestamp"]),
      cpm: numberOrNull(findXmlValue(node, ["cpm", "countsperminute", "countratecpm"])),
      cps: numberOrNull(findXmlValue(node, ["cps", "countspersecond", "countratecps"])),
      doseRate: numberOrNull(findXmlValue(node, ["doserate", "usvh", "microsieverthour", "dose"]))
    };
